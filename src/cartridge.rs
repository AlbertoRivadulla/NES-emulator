...
pub struct Rom {
    pub prg_rom: Vec<u8>, // Code of the game
    pub chr_rom: Vec<u8>, // Visuals
    pub mapper: u8,
...
impl Rom {
    pub fn new(raw: &Vec<u8>) -> Result<Rom, String> {
        // Analyze the header (16 first bytes)

        // The first 4 bytes are the string "NES^Z", used to identify .NES files.
        if &raw[0..4] != NES_TAG {
            return Err("File is not in iNES format.".to_string());
        }

        // Mapper type from the 4 highest bits of the two control bytes (6th and 7th bytes)
        let mapper: u8 = (raw[7] & 0b1111_0000) | (raw[6] >> 4);

        // The bits 2 and 3 of byte 7 must be 0 for iNES 1.0 format. iNES 2.0 format is not available.
        let ines_ver: u8 = (raw[7] >> 2) & 0b11;
        if ines_ver != 0 {
            return Err("iNES 2.0 format not supported.".to_string());
        }
        ...
        // The bit 2 of the control bit 1 is 1 if we have to skip the trainer (512 bytes before the PRG ROM)
...
pub mod test {
    ...
    fn test_nes2_is_not_supported() {
        ...
        match rom {
            ...
            Result::Err(str) => assert_eq!(str, "iNES 2.0 format not supported."),
        }
    }
}
